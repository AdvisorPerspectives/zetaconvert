...
function filterLine(line) {
  if (
    argv.opened &&
    argv.clicked &&
    line.indexOf("campaign_opened") < 0 &&
    line.indexOf("campaign_clicked") < 0
  ) {
    return false;
  }

  if (argv.opened && line.indexOf("campaign_opened") < 0 && !argv.clicked) {
    return false;
  }

  if (argv.clicked && line.indexOf("campaign_clicked") < 0 && !argv.opened) {
    return false;
  }
...
function parseInputFile() {
  ...
  let writeStream = fs.createWriteStream(outputFile, { flags: "a" }, function (
    err
  ) {
    if (err) {
  ...
  lineReader.eachLine(
    ...
    function (line) {
      if (filterLine(line)) {
        // Process only certain lines
        convertedLines++;

        // Write CSV Headers
        if (
          (!argv.append && convertedLines === 1) ||
          (argv.append && !outputFileExist)
        ) {
          writeStream.write(
    ...
    function () {
      ...
      console.log(
        outputFile +
          (!outputFileExist || !argv.append ? " created" : " appended to")
      );
      console.log(
        "Converted: " + convertedLines + " of " + totalLines + " rows"
      );
    }
  );
}
